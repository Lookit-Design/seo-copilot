...
class ASY_Processor {
    ...
    public function process( $post ) {
        ...
        if ( '' === $kp_source ) {
            if ( ! empty( $row['top_word_keyphrase'] ) )        { $kp_source = 'topword'; }
            elseif ( ! empty( $row['title_short_keyphrase'] ) ) { $kp_source = 'title_short'; }
            elseif ( ! empty( $row['slug_short_keyphrase'] ) )  { $kp_source = 'slug_short'; }
            elseif ( ! empty( $row['slug_keyphrase'] ) )        { $kp_source = 'slug'; }
            elseif ( ! empty( $row['set_keyphrase'] ) )         { $kp_source = 'title'; }
        }
        ...
        if ( ! empty( $row['ai_generate'] ) && ! isset( $row['keyphrase_source'] ) ) {
            $kp_source = 'ai'; $rel_source = 'ai'; $desc_tpl = 'ai';
        }
