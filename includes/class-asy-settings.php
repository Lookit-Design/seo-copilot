...
class ASY_Settings {
    ...
    public function sanitize_templates( $input ) {
        ...
        foreach ( $input as $post_type => $data ) {
            ...
            if ( ! in_array( $kp_src, $kp_allowed, true ) )   { $kp_src  = 'title'; }
            if ( ! in_array( $rel_src, $rel_allowed, true ) ) { $rel_src = 'off'; }
    ...
    public function enqueue_assets( $hook ) {
        ...
        if ( strpos( $hook, 'bulk-keyphrase-manager' ) !== false ) {
            ...
            wp_localize_script( 'lookit-bsm-admin', 'ASY', array(
    ...
    public function render_page( $embedded = false ) {
        ...
        <div class="<?php echo $embedded ? 'asy-wrap asy-embedded' : 'wrap asy-wrap'; ?>">
            ...
            <!-- ── Post types table ── -->
            <?php /* Real table markup (v3.29.0). Uses the Bulk Editor's proven
            ...
            <table class="bsm-table widefat bsm-auto-table">
                ...
                <tbody id="asy-rows">
                    <?php foreach ( $post_types as $pt_slug => $pt_label ) :
                        ...
                        if ( '' === $kp_source ) {
                            if ( $top_word_kp )        { $kp_source = 'topword'; }
                            elseif ( $title_short_kp ) { $kp_source = 'title_short'; }
                            elseif ( $slug_short_kp )  { $kp_source = 'slug_short'; }
                            elseif ( $slug_kp )        { $kp_source = 'slug'; }
                            else                       { $kp_source = 'title'; }
                        }
                        $rel_source = isset( $row['related_source'] ) ? $row['related_source'] : ( $ai_kp ? 'datamuse' : 'off' );
                        if ( $ai_gen && ! isset( $row['keyphrase_source'] ) ) { $kp_source = 'ai'; $rel_source = 'ai'; $tpl = 'ai'; }
                    ?>
